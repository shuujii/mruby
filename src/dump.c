...
int
mrb_dump_irep_cfunc(mrb_state *mrb, mrb_irep *irep, uint8_t flags, FILE *fp, const char *initname, mrb_bool initstatic)
{
  uint8_t *bin = NULL;
    ...
    if (fprintf(fp,
          ...
          initstatic ? "static"
                     : "#ifdef __cplusplus\n"
                       "extern\n"
                       "#endif",
          (int)MRB_DUMP_ALIGNMENT, (int)MRB_DUMP_ALIGNMENT, initname) < 0) {
      mrb_free(mrb, bin);
      return MRB_DUMP_WRITE_FAULT;
    }
